...
class POM_Affiliate_Linker {
    ...
    function add_amazon_tag( $matches ) {
        $affiliate_id = "thpoofmo0fb-20";
        $url          = parse_url( $matches[2] ); // split url into parts
        if ( isset( $url['query'] ) && strstr( $url['query'], "tag" ) ) {
            return $matches[0];
        } // ignore if already has tag query
        $prefix = isset( $url['query'] ) ? "&" : "?"; // add & if already query, ? if no query
        return str_replace( $matches[2], $matches[2] . $prefix . "tag=$affiliate_id", $matches[0] ); // add tag
    }
...
sql
        ...
        echo json_encode(['success'=>true, 'message'=>sprintf("Replaced %d urls out of %d (%0.2f%%)",$changeCount,count($has_amazon), $changeCount/count($has_amazon))]);
